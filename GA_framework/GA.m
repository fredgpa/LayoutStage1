function [ population, stats, settings, problem ] = GA( problem, settings, population )
  ...
  while (~settings.halt)
    ...
    if fx_backup ~= 0 && fx_backup ~= population.best_fx
      if abs((population.best_fx - fx_backup)) <= 0.01 * population.best_fx
        %settings.halt = true;
      end
    end
    fx_backup = population.best_fx;

    % Changes the crossover method for the next generation
    if settings.crossover_method == 1
      settings.crossover_method = 2;
    else
      settings.crossover_method = 1;
    end
  end
